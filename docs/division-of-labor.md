...
| 成員 3 | Step 2 | CNN 架構與訓練 pipeline；對「每一種去識別化參數」獨立訓練（不混訓） | `src/facedeid/model.py`、`scripts/train.py`、`scripts/train_all.py`、`scripts/plot_log.py`、`scripts/summarize_logs.py`、`config.yaml`、訓練 log（loss/acc 曲線）、各參數 `.pth` | ✅ pipeline 已補，正式訓練結果待跑 |
