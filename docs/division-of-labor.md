...
- **資料集：** `data/att_faces/` 為**真實 AT&T (ORL)**（400 張 92×112 PGM）；Step 1 去識別化變體在 `data/deid/pixelized/pix_b{2,4,8,16}/` 與 `data/deid/blurred/blur_k{15,45,99}/`；Step 3 DP 影像在 `data/dp/{dp_pix_b8,dp_pix_b16,dp_blur}/eps*/`，效用指標在 `data/dp/metrics.csv`；統一切分在 `data/splits/{split_train,split_test}.json`。**所有資料都直接在 repo，不依賴外部 zip。**
- **成員 3、4：** 訓練資料直接用 `data/deid/pixelized/pix_b{2,4,8,16}/` 與 `data/deid/blurred/blur_k{15,45,99}/`，結構與 ORL 一致（`<class>/<image>.png`），用 `facedeid.DatasetIndex.from_att(...)` 載入；切分一律 `seed=42`（各變體檔名順序相同，所以同 seed 切出來會落在同一批影像上，已驗證一致）。
- **成員 5：** DP 影像資料集與 `metrics.csv` 直接放在 `data/dp/`；本 repo 使用 `scripts/train_evaluate_dp.py` 對 DP-Pixelization / DP-Blur 重跑 CNN attack，結果整理於 `docs/dp-attack-results.md`。
