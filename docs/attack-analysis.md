...
實驗 Top-1 範圍 **86.25% 到 92.50%**，全部高於隨機 34 倍以上。即使是攻擊得最差的 `blur_k99`（k=99 的高斯模糊，肉眼幾乎看不出五官），Top-1 仍有 86.25%，Top-5 高達 98.75%。
...
|---|---:|---:|---:|---:|
| original | — | 90.00% | 98.75% | 0.4385 |
| pix_b2 | 2 | **92.50%** | 97.50% | **0.4187** |
| pix_b4 | 4 | 90.00% | 98.75% | 0.4669 |
| pix_b8 | 8 | 91.25% | 98.75% | 0.4359 |
| pix_b16 | 16 | 90.00% | **100.00%** | 0.4899 |

直覺上 b 越大、保留的細節越少，攻擊準確率應該越低。但實驗結果**全部 4 組 Top-1 都落在 90.00-92.50% 的 2.5pp 區間內**，沒有任何下降趨勢，甚至最強的 `pix_b2` (92.50%) 還略高於 `pix_b16` (90.00%)。可能原因如下：
...
### 2.2 為什麼 `pix_b2` 反而沒低於 `pix_b16`？
這很可能是**雜訊範圍內的小波動**，原因：

1. Test set 只有 80 張，1 筆預測對應 **1.25 個百分點**。`pix_b2`、`pix_b16` 之間 2.50pp 的差距只是 2 筆對錯
2. 訓練只跑了一個 seed（`seed=42`），PyTorch 在 MPS / CPU 上不完全 deterministic，不同硬體/不同 seed 之間 Top-1 容易抖動 ±2-3pp（本實驗確實觀察到多次 run 之間 `pix_b2` 在 88.75%–92.50% 之間擺動、`blur_k45` 在 88.75%–91.25% 之間擺動）
3. **真正的訊號是「整體沒下降」，不是任何一組的絕對數字**

### 2.3 Test Loss 提供補充訊號
本次 run 4 組 pix test loss 集中在 0.42-0.49（`pix_b2`=0.42、`pix_b8`=0.44、`pix_b4`=0.47、`pix_b16`=0.49），與 `original` 的 0.44 相比並沒有形成清楚的上升趨勢；模型在 pix 變體上的「信心」與原圖幾乎一致。這也呼應 Top-1 的結論：pixelization 既沒有顯著降低正確率，也沒有顯著動搖模型的 logit 分布。

### 2.4 為什麼 Top-5 動輒接近 100%？
Top-5 比 Top-1 寬鬆很多——只要正確答案在前 5 名就算對。在 40 類中佔前 5 已經把任務變得遠比 Top-1 容易，所以即使影像被破壞，CNN 輸出 logits 分布變平緩，仍很容易把正確類別擠進前 5。實際上 `pix_b16` 反而打到 100%、而 `pix_b2` 僅 97.5%，再次顯示這個指標對小幅波動很不敏感（總範圍 97.5-100%，差距只有 2 筆預測）。
...
|---|---:|---:|---:|---:|
| original | — | 90.00% | 98.75% | 0.4385 |
| blur_k15 | 15 | 91.25% | 98.75% | 0.3857 |
| blur_k45 | 45 | 88.75% | 98.75% | 0.5596 |
| blur_k99 | 99 | **86.25%** | 98.75% | **0.5859** |

Gaussian blur 的下降幅度比 pixelization 明顯：

- **`blur_k99` 是 8 組中 Top-1 最低**，與 `original` 差 3.75pp
- **Test loss 從原圖 0.44 增至 0.59**（×1.34），模型信心明顯下降
- 但 Top-5 仍維持 98.75%，模型仍能把正確類別擠進前 5

`blur_k15`、`blur_k45`、`blur_k99` 三組在當次 run 呈現嚴格單調下降（91.25% → 88.75% → 86.25%），不過 `blur_k15` 略高於 `original` 是 N=80 雜訊──**真正穩定的訊號是最強模糊 (k99) 為 8 組最低值**，這在多次獨立 run 都成立。
...
每類只有 2 筆測試，所以 per-class 只能是 0.0、0.5 或 1.0。可以看到去識別化強度越高，「完美命中」的類別數量略降（pix_b2 35 → blur_k99 31），但下降幅度有限。
...
**不應該過度解讀**：個別變體之間 1-3 個百分點的差距、pix 不單調的細節原因，需要更大規模或多 seed 才能說死。例如多次獨立 run 之間，`pix_b2` 觀察到 88.75% / 92.50% 兩個值、`blur_k45` 觀察到 88.75% / 91.25% 兩個值，這就是典型的 N=80 抖動。
...
|---|---|---:|---:|
| pixelization 最弱 | pix_b4 / pix_b16 / original | 90.00% | × 36 |
| pixelization 最強 | pix_b8 | 91.25% | × 36.5 |
| blur 最強 | blur_k99 | **86.25%** | × 34.5 |

傳統方法的**地板**仍在 86% 以上，離隨機猜測差兩個數量級。Step 3 的 ε-差分隱私引入有**理論保證**的隨機性：以 DP-Pixelization 為例，可在 ε=0.1 時把 `pix_b8` 的攻擊準確率從 91.25% 壓到約 63.75%（見 [`dp-attack-results.md`](dp-attack-results.md)），這是傳統方法不可能達到的下降幅度。

DP 的代價是影像效用下降（MSE 增、SSIM 減）。Step 3 的 ε 掃描就是在量化「隱私（攻擊準確率）vs 效用（MSE/SSIM）」這個 trade-off。

## 6. 訓練曲線觀察

各組 best epoch 集中在 **73–99** 之間（pix_b2 最早收斂在 73，blur_k45 最晚在 99），均接近 100 epoch 的訓練上限。這代表：
